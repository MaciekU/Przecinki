...
def classification_report_all(labels):
...
#not working
def optimize_hyperparameters(labels, X_train, y_train, X_test, y_test):
...
if __name__ == '__main__':
	...
	X_train = [extract_features(labels_train)]
	y_train = [get_labels(labels_train)]

	labels_test = [l for l in tagging(load_data_test_noc(), load_data_test(), dict_changer01) if l[0] != ','] 
	X_test = [extract_features(labels_test)]
	...
	classification_report_all(labi)
	...
	#optimize_hyperparameters(labi, X_train, y_train, X_test, y_test)#not working
